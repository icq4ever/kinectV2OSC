...
namespace KinectV2OSC.Model.Network
{
    public class BodySender
    {
        ...
        public BodySender(string delimitedIpAddresses, string port, string kinectID)
        {
            this.status = "";
            this.ipAddresses = this.Parse(delimitedIpAddresses);
            this.oscSenders = new List<OscSender>();
            this.port = port;
            this.kinectID = int.Parse(kinectID);
        ...
        private void Send(Body body, int kID)
        {
            ...
            // send jointMessage;
            message = messageBuilder.buildRootJointMessage(body);
            this.Broadcast(message);
            ...
            foreach (var joint in body.JointOrientations)
            {
                message = messageBuilder.buildOrientMessage(body, joint);
                this.Broadcast(message);
            }
